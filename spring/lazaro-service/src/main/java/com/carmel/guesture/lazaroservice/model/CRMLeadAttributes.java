...
public class CRMLeadAttributes {
    ...
    private String verified_c;
    private String lead_source_2_c;
    private String product_c;
    private String variant_c;
    private String uid_c;
    private String type_of_lead_c;
    private String assigned_user_id;
    private String assigned_user_name;
    private String assigned_user_link;
    public  String received_phone_number_c;

    public CRMLeadAttributes() {
    }

    public CRMLeadAttributes(Person person, CRMUser crmUser) {
        this.name = person.getName();
        this.full_name = person.getName();
        this.uid_c = person.getCupidId();
        this.phone_mobile = person.getPhoneNumber();
        if (person.getEmailAddresses() != null) {
            if (person.getEmailAddresses().length > 0) {
                this.email1 = person.getEmailAddresses()[0];
                if (person.getEmailAddresses().length > 1) {
                    this.email2 = person.getEmailAddresses()[1];
                }
            }
        }
        if (person.getSources() != null) {
            if (person.getSources().size() > 0) {
        ...
        for (Source source : person.getSources()) {
            this.description += delim + source.getMedium() + "/" + source.getPoint();
            delim = ",";
        }
        if (person.isMissed()) {
        ...
        if (person.isVerified()) {
            ...
            this.verified_c = "False";
        }
        this.lead_source = "Web Site";
        this.website = person.getRecordingURL();
        if(crmUser != null) {
            this.assigned_user_id = crmUser.getId();
            this.assigned_user_name = crmUser.getAttributes().getFull_name();
        }
        if(person.getAgents() != null){
            if(person.getAgents().size()>0){
                this.received_phone_number_c = person.getAgents().get(0).getPhoneNumber();
            }
        }
    }
    ...
    public String getVerified_c() {
        return verified_c;
    }

    public void setVerified_c(String verified_c) {
        this.verified_c = verified_c;
    }

    public String getLead_source_2_c() {
        return lead_source_2_c;
    }

    public void setLead_source_2_c(String lead_source_2_c) {
        this.lead_source_2_c = lead_source_2_c;
    }
    ...
    public String getType_of_lead_c() {
        return type_of_lead_c;
    }

    public void setType_of_lead_c(String type_of_lead_c) {
        this.type_of_lead_c = type_of_lead_c;
    }
    ...
    public String getReceived_phone_number_c() {
        return received_phone_number_c;
    }

    public void setReceived_phone_number_c(String received_phone_number_c) {
